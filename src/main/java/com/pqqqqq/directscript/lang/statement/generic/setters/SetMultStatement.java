 ...
 * A statement that sets the value of an existing {@link Variable} by multiplying by its current value
 */
public class SetMultStatement extends Statement<Object> {

    public SetMultStatement() {
    ...
    public Result<Object> run(Context ctx) {
        ...
        mutableValue.setDatum(mutableValue.getDatum().get().or(0D).mult(value)); // Null values will just be 0
